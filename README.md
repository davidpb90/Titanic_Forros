...
Project to provide a solution for Kaggle's Titanic competition
...
```bash
git clone git@github.com:manifoldai/titanic_forros.git
```
...
The runtime for titanic_forros is inside a Docker container. We have a `make` command to launch the appropriate containers. To launch the docker containers and begin working on a CPU, run from the root directory of the repository:
...
f168e19b8b67        titanic_forros_mlflow            "bash -c 'mlflow ui …"   4 days ago          Up 3 days           127.0.0.1:32770->5000/tcp   titanic_forros_mlflow_<username>
87f03baf686e        titanic_forros_bash     "/bin/bash"              4 days ago          Up 4 days           127.0.0.1:32768->8501/tcp   titanic_forros_bash_<username>
d9bd01600486        titanic_forros_jupyter   "bash -c 'cd /mnt &&…"   4 days ago          Up 3 days           127.0.0.1:32769->8888/tcp   titanic_forros_jupyter_<username>
```
...
```bash
docker exec -it titanic_forros_bash-executer_<username> /bin/bash
```
...
```bash
python titanic_forros/scripts/etl.py configs/examples/config_example.yml
```
...
```bash
python titanic_forros/scripts/evaluate.py configs/examples/config_example.yml
```
